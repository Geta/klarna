...
export default class Checkout {
    ...
    paymentMethodChange() {
        ...
        $('.jsChangePayment').each(function (i, e) {
            $(e).change(function () {
                $('.jsAddPayment, .jsPaymentTotal').show();
                $('.summary button[type="submit"]').show();
                ...
                axios.post(url, data)
                    .then(function (result) {
                        ...
                        let paymentMethod = $('[name="PaymentMethodType"]:checked').attr('keyword');

                        if (paymentMethod === 'KlarnaPayments') {
                            inst.klarnaPaymentsPaymentMethod();
                        }

                        if (paymentMethod === 'KlarnaCheckout') {
                            inst.klarnaCheckoutPaymentMethod();
                        }
                    })
                    .catch(function (error) {
                        notification.error(error);
    ...
    klarnaPaymentsPaymentMethod() {
        $('.jsAddPayment, .jsPaymentTotal').hide();

        if (window.Klarna && window.KlarnaPayments) {
            window.KlarnaPayments.load(window.KlarnaPaymentsConfiguration);
        }
    }

    klarnaCheckoutPaymentMethod() {
        $('.jsAddPayment, .jsPaymentTotal').hide();
        $('.summary button[type="submit"]').hide();
    }
    ...
    onSubmitClick() {
        let inst = this;
        $('#jsCheckoutForm').submit(function (e) {
            ...
            if (isValid && paymentMethod === 'KlarnaPayments' && $("#AuthorizationToken").val() === '') {
                e.preventDefault();
